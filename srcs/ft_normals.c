...
/*   Updated: 2020/11/12 19:10:47 by mdaifi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	plane_normal(t_env *e, t_scene *scene)
{
	...
	v = ft_vector_sub(ft_vector_add(e->curr_cam.pos, e->ray.dir), plane->pos);
...
void	square_normal(t_env *e, t_scene *scene)
{
	...
	v = ft_vector_sub(ft_vector_add(e->curr_cam.pos, e->ray.dir), square->pos);
	if (ft_vector_dot(v, scene->normal) <= 0)
		scene->normal = vector_scale(-1, scene->normal);
}
